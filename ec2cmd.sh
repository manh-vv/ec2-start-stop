...
INSTANCE_IDS=(i-040b61fe7d9fd1ed6)
...
$AWS_CLI ec2 --profile $PROFILE stop-instances $DRY_RUN --instance-ids ${INSTANCE_IDS[@]} >> $AWS_CLI_LOG 2>&1
$AWS_CLI ec2 --profile $PROFILE wait instance-stopped --instance-ids ${INSTANCE_IDS[@]} >> $AWS_CLI_LOG 2>&1
...
$AWS_CLI ec2 --profile $PROFILE start-instances $DRY_RUN --instance-ids ${INSTANCE_IDS[@]} >> $AWS_CLI_LOG 2>&1
$AWS_CLI ec2 --profile $PROFILE wait instance-status-ok --instance-ids ${INSTANCE_IDS[@]} >> $AWS_CLI_LOG 2>&1
...
$AWS_CLI ec2 --profile $PROFILE associate-address $DRY_RUN --instance-id=${INSTANCE_IDS[$count]} --public-ip=${ip} >> $AWS_CLI_LOG 2>&1
count=$(( $count + 1 ))
done
